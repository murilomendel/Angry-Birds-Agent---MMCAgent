...
public class ClientNaiveAgent implements Runnable {
	...
	private int id = 28888; // Set Agent ID
	...
	public ClientNaiveAgent(String ip) {
		...
		firstShot = true;
		dqn	= new DQN();
		///load DQN Structure from database
	}
	...
	public void run() {	
		...
		currentLevel = (byte)2;
		ar.loadLevel(currentLevel); // Load the current level
		GameState state;
		while (true) {
	...
	public GameState solve() {
		...
		dqn.preProcessImage(sling, birds, bird_sling, pigs, blocks, tnt);
		...
		if (sling != null) {
			...
			if (!pigs.isEmpty()) {		
				...
				//Pick up a target
				System.out.println("Testing Python Target Caller");
				int[] tgt = new int[2];
				
				try{
					tgt = dqn.getTargetRand();
				...
				if (pts.size() == 1) {
					...
					int shotSelect = 0;
					if (shotSelect == 2){
						System.out.println("Descobri que vale 2!!");
						//releasePoint = pts.get(1); // Try High Shot
						releasePoint = pts.get(0); // Try Low Shot
					} else {
						System.out.println("Descobri que vale 1!!");
						releasePoint = pts.get(0); // Try low shot
					}
				}
					...
					if(scale_diff < 25) {
						...
						if(dx < 0) {
							...
							if ( state == GameState.PLAYING ) {
								...
								dqn.createLog(logContent); // Creating log Content for learning task
